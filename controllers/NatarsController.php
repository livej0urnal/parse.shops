...
class NatarsController extends AppController
{
        ...
        foreach ($links as $link)
        {
            ...
            foreach ($articles as $product) {
                ...
                if(!empty($find_product)) {
                    $need_update = NatarsUpdates::find()->where(['sku_product' => $product->sku])->orderBy(['id' => SORT_DESC])->one();
                    ...
                    $new_product = new NatarsProducts();
    ...
    public function actionIndex()
    {
        ...
        $query = NatarsProducts::find()->indexBy('sku')->orderBy($sort->orders);
