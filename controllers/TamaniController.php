...
class TamaniController extends AppController
{
    public function actionLinks()
    {
        $links = Tamani::find()->all();
        foreach ($links as $link) {
            $link->delete();
        }
        $products = TamaniProducts::find()->all();
        ...
        foreach ($links as $link)
        {
            ...
            foreach ($articles as $product) {
                ...
                if(!empty($find_product)) {
                    $need_update = TamaniUpdates::find()->where(['sku_product' => $product->sku])->orderBy(['id' => SORT_DESC])->one();
    ...
    public function actionSearch($q)
    {
        ...
        $query = TamaniProducts::find()->where(['like', 'title', $q])->orWhere(['like', 'sku' , $q])->orWhere(['like', 'article' , $q])->orderBy($sort->orders);
    ...
    public function actionManufacture($q)
    {
        ...
        $query = TamaniProducts::find()->where(['like', 'article', $q])->orderBy($sort->orders);
        $pages = new Pagination(['totalCount' => $query->count(), 'pageSize' => 50, 'forcePageParam' => false, 'pageSizeParam' => false]);
        $products = $query->offset($pages->offset)->limit($pages->limit)->all();
        $manufactures = TamaniProducts::find()->select('article')->orderBy(['article' => SORT_DESC])->groupBy(['article'])->all();
        $this->setMeta('Tamani Food');
