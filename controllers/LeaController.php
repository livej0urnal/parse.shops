...
class LeaController extends AppController
{
    ...
    public function actionParse()
    {
        ...
        $links = Lea::find()->orderBy(['id' => SORT_DESC])->all();
        $parse_products = 1;
        $new_products = 0;
        $update_products = 0;
        ...
        foreach ($links as $link)
        {
            ...
            foreach ($articles as $product) {
                ...
                if(!empty($find_product)) {
                    $need_update = LeaUpdates::find()->where(['sku_product' => $product->sku])->orderBy(['id' => SORT_DESC])->one();
