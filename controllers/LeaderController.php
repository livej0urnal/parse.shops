...
class LeaderController extends AppController
{
    ...
    public function actionParse()
    {
        ...
        $links = Leader::find()->orderBy(['id' => SORT_DESC])->all();
        $parse_products = 1;
        $new_products = 0;
        $update_products = 0;
        foreach ($links as $link)
        {
            ...
            foreach ($articles as $product) {
                ...
                if(!empty($find_product)) {
                    $need_update = LeaderUpdates::findOne(['sku_product' => $product->sku]);
                    ...
                    $new_product = new LeaderProducts();
