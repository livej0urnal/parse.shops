...
use app\models\PsvUpdates;
use yii\db\Expression;
use yii\data\Pagination;

class PsvController extends AppController
{
    ...
    public function actionParse()
    {
        ...
        $links = Psv::find()->orderBy(['id' => SORT_DESC])->all();
        $parse_products = 1;
        $new_products = 0;
        $update_products = 0;
        ...
        foreach ($links as $link)
        {
            ...
            foreach ($articles as $product) {
                ...
                $find_product = PsvProducts::findOne(['sku' => $product->sku]);
                if(!empty($find_product)) {
                    $need_update = PsvUpdates::find()->where(['sku_product' => $product->sku])->orderBy(['id' => SORT_DESC])->one();
                    if(!$need_update->price) {
                        $product_update = PsvProducts::findOne(['sku' => $product->sku]);
                        $product_update->price = $product->price;
                        $product_update->instock = '1';
                        $product_update->seller = 'Psv';
                        $product_update->save(false);
                    }
                    else{
                        $new_updates = new PsvUpdates();
    ...
    {
        $id = Yii::$app->request->get('id');
        $products = PsvProducts::find()->orderBy(['id' => SORT_DESC])->limit(10)->all();
        $sort = new Sort([
            'attributes' => [
                'updated_at',
                'price',
        ...
        $query = PsvProducts::find()->indexBy('sku')->orderBy($sort->orders);
        $pages = new Pagination(['totalCount' => $query->count(), 'pageSize' => 500, 'forcePageParam' => false, 'pageSizeParam' => false]);
        $products = $query->offset($pages->offset)->limit($pages->limit)->all();
        $manufactures = PsvProducts::find()->select('article')->orderBy(['article' => SORT_DESC])->groupBy(['article'])->all();
        $this->setMeta('PSV Trade');
    ...
    public function actionSearch($q)
    {
        ...
        $products = PsvProducts::find()->where(['like', 'title', $q])->orWhere(['like', 'sku' , $q])->orderBy(['id' => SORT_DESC])->all();
        $sort = new Sort([
            'attributes' => [
                'updated_at',
                'price',
        ...
        $query = PsvProducts::find()->where(['like', 'title', $q])->orWhere(['like', 'sku' , $q])->orWhere(['like', 'article' , $q])->orderBy($sort->orders);
    ...
    {
        $q = Yii::$app->request->get('q');
        $products = PsvProducts::find()->where(['like', 'article', $q])->orderBy(['id' => SORT_DESC])->all();
        $sort = new Sort([
            'attributes' => [
                'updated_at',
                'price',
        ...
        $query = PsvProducts::find()->where(['like', 'article', $q])->orderBy($sort->orders);
        $pages = new Pagination(['totalCount' => $query->count(), 'pageSize' => 50, 'forcePageParam' => false, 'pageSizeParam' => false]);
        $products = $query->offset($pages->offset)->limit($pages->limit)->all();
        $manufactures = PsvProducts::find()->select('article')->orderBy(['article' => SORT_DESC])->groupBy(['article'])->all();
        $this->setMeta('PSV Trade');
