...
use app\models\Mamta;
use app\models\MamtaProducts;
use app\models\MamtaUpdates;
use yii\db\Expression;
use yii\data\Pagination;

class MamtaController extends AppController
{
    public function actionLinks()
    {
        $links = Mamta::find()->all();
        foreach ($links as $link) {
            $link->delete();
        }
        $products = MamtaProducts::find()->all();
    ...
    public function actionParse()
    {
        ...
        $links = Mamta::find()->orderBy(['id' => SORT_DESC])->all();
        $parse_products = 1;
        $new_products = 0;
        $update_products = 0;
            ...
            foreach ($articles as $product) {
                ...
                if(!empty($find_product)) {
                    ...
                    if(!$need_update->price) {
                        ...
                        $product_update->seller = 'Mamta';
                        $product_update->save(false);
                    }
                    else{
                        $new_updates = new MamtaUpdates();
    ...
    public function actionIndex()
    {
        ...
        $query = MamtaProducts::find()->indexBy('sku')->orderBy($sort->orders);
    ...
    public function actionSearch($q)
    {
        ...
        $products = MamtaProducts::find()->where(['like', 'title', $q])->orWhere(['like', 'sku' , $q])->orderBy(['id' => SORT_DESC])->all();
        $sort = new Sort([
            'attributes' => [
                'updated_at',
                'price',
        ...
        $query = MamtaProducts::find()->where(['like', 'title', $q])->orWhere(['like', 'sku' , $q])->orWhere(['like', 'article' , $q])->orderBy($sort->orders);
    ...
    {
        $q = Yii::$app->request->get('q');
        $products = MamtaProducts::find()->where(['like', 'article', $q])->orderBy(['id' => SORT_DESC])->all();
        $sort = new Sort([
            'attributes' => [
                'updated_at',
                'price',
        ...
        $query = MamtaProducts::find()->where(['like', 'article', $q])->orderBy($sort->orders);
        $pages = new Pagination(['totalCount' => $query->count(), 'pageSize' => 50, 'forcePageParam' => false, 'pageSizeParam' => false]);
        $products = $query->offset($pages->offset)->limit($pages->limit)->all();
        $manufactures = MamtaProducts::find()->select('article')->orderBy(['article' => SORT_DESC])->groupBy(['article'])->all();
        $this->setMeta('Mamtakim Inc.');
