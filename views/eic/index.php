        ...
        <div class="row">
            <div class="col-md-12">
                <div class="au-breadcrumb-content">
                    <form class="au-form-icon--sm" action="<?= \yii\helpers\Url::to(['eic/search']) ?>" method="get" >
                    ...
                    <?php if(!empty($manufactures)) : ?>
                        <div class="col-md-4">
                            <select name="select" id="select-manufacture" class="form-control" data-value="eic">
                    ...
                    <table class="table table-data2">
                        ...
                        <?php if(!empty($products)) : ?>
                            ...
                            <?php foreach ($products as $product) : ?>
                                <?php $last_update = \app\models\EicUpdates::find()->where(['sku_product' => $product->sku])->andWhere(['!=', 'price' , $product->price])->orderBy(['update_at' => SORT_DESC])->one(); ?>
                                <tr class="tr-shadow find-gmi-updates <?php if(!empty($last_update)) :  ?>mark<?php endif; ?>" data-value="<?= $product->sku ?>">
                                    ...
                                    <td><?= $product->units ?></td>
                                    <td><?= $product->per ?></td>
                                    <td>$<?= $product->price ?> <?php if(!empty($last_update)) :  ?><br><span class="mark title--sbold" style="color: red"><?php echo $product->price - $last_update->price;  ?></span><?php endif; ?></td>
                                    <td><?php if($product->instock === null) : ?> <span style="color:red;">out</span> <?php else : ?> <span style="color:green;">in</span> <?php endif; ?></td>
                                    <td><?= $product->seller ?></td>
                                </tr>
                                <?php $updates = \app\models\EicUpdates::find()->select(['price' , 'update_at', 'sku_product'])->where(['sku_product' => $product->sku])->orderBy(['update_at' => SORT_ASC])->all(); ?>
